...
(function () {
	...
	const DragScroll = {
		init: function () {
			const containers = document.querySelectorAll(".hv-artists-showcase, .hv-services-grid");
...
const GalleryFilter = {
	init: function () {
		const self = this;
		const filters = document.querySelectorAll(".hv-gallery-filters__btn");
		const items = document.querySelectorAll(".hv-gallery-bento__item");

		if (!filters.length || !items.length) return;

		filters.forEach(function (btn) {
			btn.addEventListener("click", function () {
				const id = this.getAttribute("data-artist-id");
				...
				items.forEach(function (el) {
					const match = id === "0" || el.getAttribute("data-artist-id") === id;
					if (match) {
						el.classList.remove("hv-gallery-bento__item--hidden");
						...
						el.classList.add("hv-gallery-bento__item--hidden");
						el.style.display = "none"; // Hide completely to fix layout gaps
					}
				});

				A11yUtils.announce("Refreshed gallery. Showing " + visibleCount + " items.");
			});
		});
	}
};
