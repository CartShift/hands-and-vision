			...
			document.querySelectorAll(".hv-artist-card-premium__expander").forEach(btn => {
				btn.addEventListener("click", function (e) {
					...
					if (isExpanded) {
						...
						this.innerHTML =
							'<span class="hv-expander-text">' +
							(isHebrew ? "קרא עוד" : "Read More") +
							'</span><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M19 9l-7 7-7-7"/></svg>';
					} else {
						...
						this.innerHTML =
							'<span class="hv-expander-text">' +
							(isHebrew ? "סגור" : "Close") +
							'</span><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 15l7-7 7 7"/></svg>';
					}
				});
			});
		}
